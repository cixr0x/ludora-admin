...
class DiscoveryRepository:
        ...
        *,
        ...
        with self.connection.cursor() as cursor:
                    ...
                    where stores.active = true
                      ...
                      and not exists (
                        ...
                        where cooldown.worker_name = %s
                          and cooldown.store_id = store_items.store_id
            ...
            cursor.execute(
                ...
                from store_items
                join stores on stores.id = store_items.store_id
                left join store_item_update_store_cooldown cooldown
        ...
        return ClaimedStoreItemUpdate(
            ...
            platform=platform,
            record=record,
            store_consecutive_429s=int(store_row[3]) if store_row else 0,
    ...
    def complete_claimed_store_item_update(
        ...
        *,
        ...
        with self.connection.cursor() as cursor:
            ...
            cursor.execute(
                """
                update store_item_update_store_cooldown
    ...
    def deactivate_claimed_redirected_store_item_update(
        self,
        existing_record: DiscoveryItemCandidateRecord,
        *,
        attempt_id: int,
        ...
        with self.connection.cursor() as cursor:
            ...
            locked_ids = tuple(sorted((store_item_id, target_store_item_id)))
            cursor.execute(
                """
                select id
                from store_items
            ...
            if [int(row[0]) for row in cursor.fetchall()] != list(locked_ids):
                self.connection.commit()
                return None
            ...
            cursor.execute(
                ...
                join store_items target_item on target_item.id = %s
                where source_item.id = %s
                  and source_item.store_id = %s
                  and source_item.item_id = %s
                  and source_item.item_id is not null
                  and source_item.store_active = true
                  and source_item.update_lease_token = %s::uuid
                  and target_item.id <> source_item.id
                  and target_item.store_id = source_item.store_id
                ...
                """,
                (target_store_item_id, store_item_id, store_id, item_id, lease_token),
            )
            rechecked_target = cursor.fetchone()
            if (
                rechecked_target is None
                or int(rechecked_target[0]) != target_store_item_id
                or normalize_store_item_url(_text(rechecked_target[1])) != normalized_final_url
            ):
                self.connection.commit()
                return None

        self.deactivate_claimed_store_item_update(
            existing_record,
    ...
    def fail_claimed_store_item_update(
        ...
        *,
        ...
        store_blocked_until: datetime | None,
        worker_id: str,
        worker_name: str,
        job_id: int,
        platform: str,
        ...
        with self.connection.cursor() as cursor:
            ...
            if (
                ...
                and http_status == 429
                and store_blocked_until is not None
            ):
                cursor.execute(
                    """
                    insert into store_item_update_store_cooldown (
                        worker_name,
                        store_id,
                        blocked_until,
                        consecutive_429s,
                    ...
                    values (%s, %s, %s, 1, now())
                    on conflict (worker_name, store_id) do update set
                        blocked_until = greatest(
                          store_item_update_store_cooldown.blocked_until,
                          excluded.blocked_until
                        ),
                        consecutive_429s = store_item_update_store_cooldown.consecutive_429s + 1,
                        updated_at = now()
                    """,
                    (worker_name, existing_record.store_id, store_blocked_until),
                )
            cursor.execute(
                """
                update store_item_update_worker_state
