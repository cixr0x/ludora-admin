...
def resolve_ai_classifier_enabled(
    env: Mapping[str, str] | None = None,
    dotenv_path: str | Path = ".env",
) -> bool:
    current_env = env if env is not None else os.environ
    env_value = current_env.get("AI_ENABLED_CLASSIFIER", "").strip()
    if env_value:
        return _is_truthy(env_value)

    dotenv_value = load_dotenv_values(dotenv_path).get("AI_ENABLED_CLASSIFIER", "").strip()
    if dotenv_value:
...
def resolve_internal_api_token(
    env: Mapping[str, str] | None = None,
    dotenv_path: str | Path = ".env",
) -> str:
    current_env = env if env is not None else os.environ
    env_value = current_env.get("LUDORA_INTERNAL_API_TOKEN", "").strip()
    if env_value:
        return env_value

    return load_dotenv_values(dotenv_path).get("LUDORA_INTERNAL_API_TOKEN", "").strip()
