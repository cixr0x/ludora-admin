...
class SchemaTests(unittest.TestCase):
    ...
    def test_store_items_replaces_listing_candidates(self):
        ...
        self.assertIn("refreshed_date timestamptz not null default now()", item_candidate_table)
        self.assertIn("alter table if exists store_items add column if not exists refreshed_date timestamptz", schema)
        self.assertIn("update store_items set refreshed_date = last_updated where refreshed_date is null", schema)
        self.assertIn("alter table if exists store_items alter column refreshed_date set default now()", schema)
        self.assertIn("alter table if exists store_items alter column refreshed_date set not null", schema)
    ...
    def test_schema_contains_store_item_update_change_log(self):
        ...
        self.assertIn("create table if not exists store_item_update_change_log", schema)
    ...
    def test_schema_contains_active_item_view(self):
        ...
        self.assertIn("from store_items primary_store_item", view)
        self.assertIn("primary_store_item.item_id is not null", view)
        self.assertIn("union all", view)
        self.assertIn("from store_item_additional_items additional_link", view)
        self.assertIn("join store_items bundled_store_item", view)
        self.assertIn("bundled_store_item.id = additional_link.store_item_id", view)
        self.assertIn("bool_or(", view)
        self.assertIn("group by eligible_item_links.item_id", view)
        self.assertIn("join items catalog_item", view)
        self.assertIn("catalog_item.id = active_item_links.item_id", view)
        self.assertIn("as has_approved_listing", view)
        self.assertIn("eligible_item_links.listing_status = 'listed'", view)
        self.assertIn("as is_expansion", view)
        self.assertIn("relationship.link_type = 'extension'", view)
