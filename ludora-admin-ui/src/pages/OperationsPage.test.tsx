...
describe('OperationsPage', () => {
  ...
  it('starts cover image optimization, polls its run, and renders the completed summary', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      const url = String(input);
      if (url.endsWith('/admin/operations/external-cover-image-optimizations/latest')) {
        return jsonResponse({ data: null });
      }
      ...
      if (url.endsWith('/admin/operations/external-cover-image-optimizations/optimization-1')) {
        return jsonResponse({
          data: {
            completed_at: '2026-08-04T02:55:00Z',
            error: null,
            id: 'optimization-1',
            result,
            started_at: '2026-08-04T02:45:00Z',
    ...
    const optimizeButton = await screen.findByRole('button', { name: /Optimize Cover Images/i });
    await waitFor(() => expect(optimizeButton).toBeEnabled());
    await user.click(optimizeButton);

    expect(await screen.findByText('Cover image optimization is running. Status will refresh automatically.')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Optimize Cover Images/i })).toBeDisabled();

    expect(await screen.findByText('Image optimization summary', {}, { timeout: 4500 })).toBeInTheDocument();
    expect(screen.getByText('Cover image optimization completed successfully.')).toBeInTheDocument();
