...
describe('ListingCandidatesPage', () => {
  ...
  it('searches by item names and associates a store item from the table', async () => {
    const user = userEvent.setup();
    const originalCandidate = {
      id: '3365',
      item_id: null,
      source_url: 'https://www.amigocalavera.mx/productos/aeterna/',
      store_id: 42,
      title: 'Aeterna'
    };
    const catalogItem = {
    ...
    let currentCandidate: Record<string, unknown> = originalCandidate;
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      if (pathOf(url) === '/items' && !init?.method) {
        return jsonResponse([catalogItem], 200, { page: 0, page_size: 8, total: 1 });
      }
      if (pathOf(url) === '/discovery/listings/3365/associate-item' && init?.method === 'POST') {
        currentCandidate = {
          ...currentCandidate,
          is_boardgame: true,
          is_boardgame_confirmed: true,
        ...
        return jsonResponse(currentCandidate);
      }
      throw new Error(`Unexpected request: ${url}`);
    });
    ...
    expect(await screen.findByText('Aeterna')).toBeInTheDocument();
    await user.click(screen.getByRole('button', { name: 'Associate Aeterna with an existing item' }));
    ...
    const itemSearchRequest = fetchMock.mock.calls.find(([url]) => pathOf(String(url)) === '/items');
