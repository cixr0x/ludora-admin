...
describe('auto-list evaluation service', () => {
  ...
  it('requires nearly identical cover artwork, same-language names, and all checks to pass', () => {
    ...
    expect(prompt).toContain('Ignore ordinary retailer wording, but require all meaningful product qualifiers such as "Module", "Duel", "Junior", or their same-language equivalents to correspond.');
    ...
    expect(prompt).not.toContain('The standalone English word "Expansion" is optional');
    expect(prompt).toContain('The standalone word "Expansion" or "Expansión" is optional when it is the only difference between the same-language store and catalog titles, case-insensitively');
    expect(prompt).toContain('regardless of which of the two compared titles contains it');
    expect(prompt).toContain('ignore parentheses and punctuation surrounding only that optional word');
    expect(prompt).toContain('catalog "The Red Cathedral: Contractors" and store "The Red Cathedral: Contractors (Expansion)" must pass nameMatches');
    expect(prompt).toContain('store "The Red Cathedral: Contractors" and catalog "The Red Cathedral: Contractors (Expansion)" must pass nameMatches');
    expect(prompt).toContain('Likewise, catalog "La Catedral Roja: Contratistas" and store "La Catedral Roja: Contratistas (Expansión)" must pass nameMatches.');
    expect(prompt).toContain('This exception does not allow any other missing or extra meaningful word or qualifier; Module, Duel, Junior, edition names, actual expansion names, and other identity-changing text remain mandatory');
    expect(prompt).toContain('sameGame and the overall verdict remain strict');
