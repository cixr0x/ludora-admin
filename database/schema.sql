...
create materialized view if not exists active_item as
...
create unique index if not exists active_item_id_uidx
on active_item (id);

create table if not exists active_item_refresh_state (
...
create or replace function request_active_item_refresh()
returns trigger
language plpgsql
security definer
...
create or replace function refresh_active_item_if_needed()
returns boolean
language plpgsql
security definer
set search_path = pg_catalog, public
